...
public class Test {
    ...
    public void  backTrack(List<List<String>> result,int row,int n ,String[][] current,Set<Integer>  columnSet,Set<Integer> diagonalSet1,Set<Integer> diagonalSet2){
          if (row == n){
               result.add(transfer(current));
          }else {
              for (int column = 0; column <n ; column++) {
                  if (columnSet.contains(column) || diagonalSet1.contains(column - row)|| diagonalSet2.contains(column+row)){
                      continue;
                  }
                  // 该位置符合要求，放置棋子，并在序号集合中 占用 行 ，主对角线  次对角线的序号
